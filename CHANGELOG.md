...
- **Per-group serving speed.** `ncl groups config update --id <group> --speed fast` selects a serving tier the group's provider declares; Claude accepts `standard` and `fast`. Fast mode costs more per token; `--speed ""` clears it. Agent callers need approval, as with `--timezone`.
- **Iron Proxy gateway.** `/add-iron-proxy` installs Iron Proxy and its Iron Control web console (Secrets, Credentials, OAuth Apps, Principals) as an alternative local gateway on Docker. Each request passes front identity and allowlist, then human approval, then stored credentials, before it reaches upstream. Codex works through Iron, including its WebSocket turns. See [.claude/skills/add-iron-proxy/SKILL.md](.claude/skills/add-iron-proxy/SKILL.md). Setup can install Codex and pair it under Iron in one run, and a failed credential save names its cause on screen and in `logs/setup.log`.
- **OpenCode can authenticate through Iron Control.** OpenCode can store API keys and native ChatGPT OAuth credentials through Iron Control, including token refresh and reauthentication that preserves grants. Existing OpenCode installs must refresh the OpenCode payload, rebuild the agent image, and restart.
- **Echo's hardened image and a managed Slack app are set up through the community portal with one browser sign-in.** Setup prints one link that covers sign-in, terminal approval and the perk choice; the install token never passes through the browser. The running host stays linked to your account's cell, so a perk changed in the browser reaches the running agent and a Slack install approved days later finishes on its own. Setup asks each portal question once, including across a resumed or repeated run. Dial and Tavily are not offered.
- **OpenCode provider rework.** The optional `/add-opencode` skill now uses the native provider contracts: host-managed authentication and model selection, native tools and compaction, and reliable cancellation. Setup can install and authenticate it. **Existing OpenCode installs must refresh the skill payload** — re-run `/add-opencode`, then rebuild the agent image.
- **Mattermost channel.** `/add-mattermost` installs a Mattermost channel. It can connect to a server you already run, validates and atomically saves its settings, and verifies the running bot and server callback. It no longer needs a separately installed `jq`.
- **Chat SDK channels can recover content the adapter left in the raw payload.** The bridge drops `message.raw` before persisting, so anything a platform did not project into its message text was lost — Slack keeps pasted tables in `attachments[].blocks[]`, and an agent saw only the sentence before the table. A new optional `extractRawText` hook on `ChatSdkBridgeConfig` lets a channel rescue that content as text before raw is discarded. Raw payloads still never reach the database, and a channel that does not set the hook is unaffected. Slack uses it to recover pasted tables; refresh `/add-slack` to get it.
- **Replies stay in the thread they answer.** Replies, `send_message`, `send_file`, `ask_user_question` and `send_card` now land in the thread of the message being answered, not the main channel or a thread that spoke more recently. This covers shared and agent-shared sessions, and replies from a turn longer than 30 minutes stay threaded.
- **Busy installs stay fast.** The host sweep and delivery polls visit up to 8 sessions at a time at a fixed rate, and cross-session ambient context fans out only to a conversation's 8 most recently active threads (within 3 days), after the container wakes. Installs with many sessions or busy channels no longer see reply and task latency grow with size.
- **Host restarts keep approvals and delivery state.** Approvals, delivery retry counts and session claims now live in the central database instead of host memory. An approval card sent before a restart still resolves after it (a late approve that can no longer be applied gets a follow-up card), overdue approvals expire as "no response" instead of being swept as "host restarted", and a message that keeps failing gives up after its retry limit even across a crash loop instead of retrying forever. A second host started in the same checkout no longer takes over a running host's `ncl` socket, and a host that is still draining during a restart keeps its sessions until it exits.
...
- `/update-nanoclaw` loads its controller again (it crashed with `MODULE_NOT_FOUND`), snapshots symlinked data roots by content, and is correct on macOS hosts. Cutover no longer times out while an idle agent container is running: it stops this install's agent containers itself (10-second grace, one-minute bound) and names them as it goes, so no manual `docker stop` is needed. An agent in the middle of a turn loses that turn, as with `ncl groups restart`.
- Setup is sturdier on Linux: no hang with a system-wide Node, a user-owned npm prefix instead of an invisible sudo, a nohup-started host verified and kept running when user systemd is missing, installers run under `/bin/sh` and `/bin/bash`, a uvx-installed pnpm recovered, and the replacement host verified after a restart.
- Setup redacts resolved secrets in skill logs and commits related `.env` values atomically.
- Concurrent SQLite migrations no longer fail setup or upgrade; failed registry copies keep existing files; registry skills install from single-branch clones.
- OneCLI certificate and credential-stub mounts survive host restarts and temp cleanup, and `WEBHOOK_PORT` is read from `.env`.
- Cancelled questions stop promptly, failed provider turns keep partial replies, scheduled-task escalations default to the agent's own channel, and Slack group-DM and multi-instance approval cards name the right conversation and bot, and installing missing skill files keeps your customizations.
- `ncl tasks update` rejects an empty `--prompt`, and a timed-out pre-task script says so.
- The Slack agents companion skills (`/slack-a2a-rooms`, `/slack-agent-flow`) now ship on `main`.
- The agent image is repinned (hardened-2026-08-24) and runs Bun 1.4.0, and pnpm's 3-day `minimumReleaseAge` supply-chain gate is now enforced.
...
- `/add-dial` installs the Dial tool on a first-ever install (before any agent exists), echoes what you type at its agent-scope question, and `/add-dial-tool` can re-register a rotated key with OneCLI.
- `/add-opencode` documents `OPENCODE_MODEL_CONTEXT_LIMIT`, `OPENCODE_MODEL_OUTPUT_LIMIT` and `OPENCODE_MODEL_INPUT_MODALITIES`, which set a custom model's context, output and input limits.
- Agents now explain that `send_card` supports URL link buttons, and that interactive choices need `ask_user_question`.
