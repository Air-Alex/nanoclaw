...
- **Claude agents default to a concise tone.** Claude groups now start with the `Concise` output style unless the group's own settings already set one; an explicit output style is never overwritten. ⟦revert knob — confirm with zvi: `outputStyle` in the group's `.claude/settings.json`?⟧
...
- `/update-nanoclaw` loads its controller again (it crashed with `MODULE_NOT_FOUND`), snapshots symlinked data roots by content, and is correct on macOS hosts. ⟦if #3873 merges, add: "Cutover no longer times out while an idle agent container is running."⟧
