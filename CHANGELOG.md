...
- **Echo's hardened image and a managed Slack app are set up through the community portal with one browser sign-in.** Setup prints one link that covers sign-in, terminal approval and the perk choice; the install token never passes through the browser. The running host stays linked to your account's cell, so a perk changed in the browser reaches the running agent and a Slack install approved days later finishes on its own.
...
- **Mattermost channel.** `/add-mattermost` installs a Mattermost channel. It can connect to a server you already run, validates and atomically saves its settings, and verifies the running bot and server callback.
...
- **Replies stay in the thread they answer.** Replies, `send_message`, `send_file`, `ask_user_question` and `send_card` now land in the thread of the message being answered, not the main channel or a thread that spoke more recently.
...
- Cancelled questions stop promptly, failed provider turns keep partial replies, scheduled-task escalations default to the agent's own channel, and Slack group-DM and multi-instance approval cards name the right conversation and bot.
